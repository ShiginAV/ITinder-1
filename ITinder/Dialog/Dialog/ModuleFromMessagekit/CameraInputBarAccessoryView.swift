...
extension CameraInputBarAccessoryView : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @objc func showImagePickerControllerActionSheet() {
        ...
        let alert = AlertService.showAlert(style: .actionSheet, title: "Choose Your Image", message: nil, actions: [photoLibraryAction, cameraAction, cancelAction])
        (delegate as? CameraInputBarAccessoryViewDelegate)?.showAlert(alert: alert)
    }
    ...
    func getRootViewController() -> UIViewController? {
        return (delegate as? CameraInputBarAccessoryViewDelegate)?.getSelf()
    }
}
